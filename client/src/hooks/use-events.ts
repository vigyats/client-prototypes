...
export function useEvents(params?: { lang?: "en" | "hi" | "mr" }) {
  const qs = new URLSearchParams();
  if (params?.lang) qs.set("lang", params.lang);
  const url = `${api.events.list.path}${qs.toString() ? `?${qs.toString()}` : ""}`;

  return useQuery<EventListResponse>({
    queryKey: [api.events.list.path, params?.lang || ""],
    queryFn: async () => {
...
export function useEvent(id: number, params?: { lang?: "en" | "hi" | "mr" }) {
  ...
  return useQuery<EventGetResponse>({
    queryKey: [api.events.get.path, id, params?.lang || ""],
    queryFn: async () => {
      const res = await fetch(url, { credentials: "include" });
...
export function useUpdateEvent() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      qc.invalidateQueries({ queryKey: [api.events.get.path, vars.id] });
    },
  });
}
